...
Force curves which can not be corrected are discarded by SOFA. To shift the point of contact of a single force curve to zero, first its :ref:`force values <correction force>` and then the :ref:`pizeo values <correction piezo>` are corrected.
...
.. figure:: images/restrict_search_area.svg
	...
	Restriction of the search area with all the auxiliary points.
...
To correct the topography offset the force distance curve is shifted to zero along the piezo (x) axis using the point of contact. This can only be done after the correction of the deflection values, since the point of contact is defined as the first zero crossing after the jump to contact. First the :ref:`zero crossing <calculate zero crossing>` is determined. Afterwards the exact :ref:`point of contact is interpolated <interpolate point of contact>`. Finally the pizeo values of the approach curve can be :ref:`shifted using the point of contact <shift piezo values>`.
...
To locate the first zero crossing, the last measuring point with a negative deflection value after the end of the zero line is searched for. If there are no points with negative deflection values after the end of the zeroline, the force distance curve cannot be corrected and the correction process is aborted for this curve.
...
With the last measurement point before and after the zero crossing the exact piezo value at which the deflection becomes zero is calculated. 
